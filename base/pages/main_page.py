...
class MainPage(BasePage):
    ...
    def set_email_input(self, email):
        element = self.wait_by_xpath("//input[@id='username']")
        element.send_keys(email)

    def set_password_input(self, password):
        element = self.wait_by_xpath("//input[@id='password']")
        element.send_keys(password)

    def click_login2(self):
        element = self.get_by_xpath("//button[text()='Continue']")
        element.click()

    def wait_home_page(self):
        element = self.wait_by_id('explore-header')
